 ...
 *
 * Any authenticated user may upload any file type (images, video, PDF, docs,
 * archives, ...). addRandomSuffix guarantees unique pathnames so re-uploading a
 * file with the same name never throws "This blob already exists".
 */
  ...
  try {
    ...
    const result = await handleUpload({
      ...
      onBeforeGenerateToken: async (_pathname, clientPayload) => {
        ...
        return {
          // No allowedContentTypes => every file type is accepted.
          maximumSizeInBytes: 500 * 1024 * 1024, // 500 MB (large videos, etc.)
